...
import DrawerProductWriteFrom from './components/DrawerProductWriteFrom';
...
const Sort = () => {
  ...
  const [selectedAdvert, setSelectedAdvert] = useState<Advert | undefined>(
    undefined,
  );
  ...
  const handleOpenDeleteModal = (advert: Advert) => {
  ...
  const handleOpenUpdateForm = (advert: Advert) => {
    setSelectedAdvert(advert);
    openUpdateForm();
  };
    ...
    <Stack>
      ...
      <Grid columns={10}>
        <Grid.Col sm={10} md={4}>
          <TextInput
            ...
            placeholder='Search advert by sort name'
            value={search}
      ...
      >
        {sorts.map((advert) => (
          <SortCard
            key={advert.id}
            img={advert.sort.imageUrl ? advert.sort.imageUrl : ""}
            name={advert.sort.name}
            volume={advert.qualitiesOfAdvert[0].volume}
            price={advert.qualitiesOfAdvert[0].priceSell}
            handleEdit={() => handleOpenUpdateForm(advert)}
            handleDelete={() => handleOpenDeleteModal(advert)}
          />
        ))}
      </SimpleGrid>

      {selectedAdvert && (
        <DrawerProductWriteFrom
