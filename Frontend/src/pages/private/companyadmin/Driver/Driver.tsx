      ...
      {isSuccess && driver && (
        <>
          <Card shadow='sm' p='lg' radius='md' withBorder>
            <Group position='apart'>
              <Stack spacing='xs'>
                ...
                <Text size='sm' color='dimmed'>
                  Данные страховки: {driver.InsuranceData}
                </Text>
                <Text size='sm' color='dimmed'>
                  Номер страховки: {driver.InsuranceNumber}
                </Text>
                <Text size='sm' color='dimmed'>
                  Дата ДТП:{' '}
                  {driver.DtpDate
                    ? new Date(driver.DtpDate).toLocaleDateString()
                    : 'Нет данных'}
                </Text>
                ...
                <Text size='xs' color='dimmed'>
                  Обновлено: {new Date(driver.UpdatedAt).toLocaleDateString()}
                </Text>
              </Stack>
              <Stack align='center'>
                <Text size='sm' color='dimmed'>
          ...
          <Flex direction='column' mt='xl'>
            <Title order={3}>Отзыв компании</Title>
            {userRating ? (
          ...
          0 ? (
            ...
            <EmptyState
              mt='xl'
              title='Нет отзывов'
              description='Для этого водителя нет отзывов от других компаний.'
              Icon={
                <Avatar radius='100%' size='xl' variant='light' color='red'>
                  <IconFileAlert size={25} />
                </Avatar>
